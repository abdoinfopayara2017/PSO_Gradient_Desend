...
def creat_file_dataset() :
  for subsetindex in range(10):
      
   path = "E:\LUNA 16\classification\subset" + str(subsetindex) + "/1/1_aug/"
   ...
   # For images
    ...
    with open('dataset.txt', 'a') as f:
       print('1 ' + images_path, file=f)
    
   path = "E:\LUNA 16\classification\subset" + str(subsetindex) + "/0/"
   file_list = glob(path + "*.npy")
...
def creat_dataset_file():   
    filepath = 'E:\LUNA 16\classification\dataset_all'
    
    if not os.path.exists(filepath):
        os.makedirs(filepath)
    paths = [line.split(None,1)[1] for line in open('training_data.txt','r')]
    for i in range(len(paths)):
        path = paths[i].split('classification',1)[1]
        path = path[:path.rfind('\\')]        
        cmd = 'copy "'+ paths[i].strip() +'" "'+filepath + path +'"'
        if not os.path.exists(filepath + path):
         os.makedirs(filepath + path)        
        os.system(cmd)
        #shutil.copyfile(paths[i], filepath + path)
